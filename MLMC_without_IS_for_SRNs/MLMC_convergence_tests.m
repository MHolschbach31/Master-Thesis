...
%% defining the total number of samples as well the deepest level for MLMC, for each example in example.m  
 ...
 %% writing the outputs in .txt file
  ...
  filename = ['Example_x7_' num2str(exp)];
...
%% print  reference value
  ...
  if (exp==1)
   val=0.3296;   % computed analytically
  elseif (exp==2)
...
%
  filename = ['mlmc_tau_leap_l_x7_' num2str(exp) '_100'];
  fp = fopen([filename '.txt'],'w');
...
for N1 = 1:10000:N
  ...
  if exp==1
    ...
    %fl1 = figure;
    %h = histogram(Pf)
    %title('Rare event histogram for finest level')
    %saveas(fl1,'histogram_decay_finest.png')
    %fl2 = figure;
    %h = histogram(dP)
    %title('Rare event histogram for difference')
    %saveas(fl1,'histogram_decay_difference.png')
    end
  ...
  if l==0
    ...
    f = figure;
    h = histogram(dP);
    title('Abundance of species at final time for example 1')
    saveas(f,'histogram_decay_x1_test.png')
